...
#include "ui_zcottoncontroldialog.h"
...
std::list<std::string> ZCottonControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZCottonControlDialog::listMints;

ZCottonControlDialog::ZCottonControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZCottonControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZCottonControlDialog::~ZCottonControlDialog()
{
    delete ui;
}

void ZCottonControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCottonControlDialog::updateList()
{
...
void ZCottonControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCottonControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZCotton_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCottonControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZCottonControlDialog::GetSelectedMints()
{
...
void ZCottonControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
