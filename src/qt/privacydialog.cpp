...
#include "zcottoncontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCOTN ought to be enough for anybody." - Bill Gates, 2017
    ui->zCOTNpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCOTNSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzCOTN->setEnabled(false);
        ui->pushButtonMintzCOTN->setToolTip(tr("zCOTN is currently disabled due to maintenance."));

        ui->pushButtonSpendzCOTN->setEnabled(false);
        ui->pushButtonSpendzCOTN->setToolTip(tr("zCOTN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCOTNpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCOTN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCOTN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCOTN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCOTN in ") +
...
void PrivacyDialog::on_pushButtonSpendzCOTN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCOTN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCOTN
        sendzCOTN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCOTN
    sendzCOTN();
}

void PrivacyDialog::on_pushButtonZCottonControl_clicked()
{
    ZCottonControlDialog* zCOTNControl = new ZCottonControlDialog(this);
    zCOTNControl->setModel(walletModel);
    zCOTNControl->exec();
}

void PrivacyDialog::setZCottonControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCOTNSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCOTN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Cotton Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCOTNpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCOTN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " Cotton </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCOTN</b>";
    ...
    // use mints from zCOTN selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCottonControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZCottonControlDialog::GetSelectedMints();
    }
    ...
    // Clear zcotton selector in case it was used
    ZCottonControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCOTN Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Cotton, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zCOTN Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCOTN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCOTN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCOTN "));
    ui->labelzCOTNAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzCOTNSyncStatus->setVisible(fShow);
}
