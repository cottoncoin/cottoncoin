...
// Copyright (c) 2017-2018 The Cotton developers
...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total cotton balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zCOTNsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zCOTN denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zCOTN denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zCOTN denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zCOTN denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zCOTN denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zCOTN denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zCOTN denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zCOTN denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zCOTN denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in cotton/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in cotton/kb\n"
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zcottonObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zcottonObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zCOTNsupply", zcottonObj));
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"cottonaddress\"\n"
            "\nReturn information about the given cotton address.\n"
            "\nArguments:\n"
            "1. \"cottonaddress\"     (string, required) The cotton address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"cottonaddress\", (string) The cotton address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Cotton address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are cotton addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) cotton address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"cottonaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"cottonaddress\"  (string, required) The cotton address to use for the signature.\n"
