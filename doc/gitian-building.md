...
*Setup instructions for a gitian build of Cotton using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Cotton
...
- [Building Cotton](#building-cotton)
...
Clone the git repositories for cotton and gitian and then checkout the cotton version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the cotton repository
...
To build Cotton (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the cotton repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cotton/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/eastcoastcrypto/Cotton
...
For example:
```bash
URL=https://github.com/eastcoastcrypto/Cotton.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit cotton=${COMMIT} --url cotton=${URL} ../Cotton/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cotton=${COMMIT} --url cotton=${URL} ../Cotton/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cotton=${COMMIT} --url cotton=${URL} ../Cotton/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cotton-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cotton-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/cotton-build.assert
```
